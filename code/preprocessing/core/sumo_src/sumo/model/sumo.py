...
class SUMO(pl.LightningModule):
    ...
    def __init__(self, config: Config):
        ...
        self.encoders = nn.ModuleList(
            [
                Encoder(chs * 2**i, chs * 2 ** (i + 1), pools[i], **model_params)
        ...
        self.decoders = nn.ModuleList(
            [
                Decoder(chs * 2 ** (i + 1), chs * 2**i, pools[i], **model_params)
    ...
    def dense_logits(self, x: torch.Tensor) -> torch.Tensor:
        ...
        extrapolation = int(
            np.ceil(n_samples / np.prod(self.pools)) * np.prod(self.pools) - n_samples
        )
        # extrapolation is done using the reflection/mirroring mode
        x = F.pad(
            x,
            (extrapolation // 2, extrapolation // 2 + extrapolation % 2),
            mode="reflect",
        )
    ...
    def get_batch_results(self, logits: torch.Tensor, mask: torch.Tensor):
        ...
        spindles_pred_batch = (
            F.softmax(averaged_logits, dim=1)
            .argmax(dim=1)
            .long()
            .detach()
            .cpu()
        ...
        return {
            "detected": n_spindles_detected,
            "gold_standard": n_spindles_gs,
            "tp": n_true_positives,
        }
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.criterion(logits, mask)
        self.log(
            "loss/val", loss, on_step=False, on_epoch=True, prog_bar=True, logger=True
        )
    ...
    def on_validation_epoch_end(self):
        ...
        n_true_positives = np.array(
            [x["tp"] for x in self.validation_step_outputs]
        ).sum(axis=0)
    ...
    def crop(x, output_samples):
        ...
        len_x = x.shape[-1]
        assert (
            output_samples <= len_x
        ), f"x should contain at least output_samples, but contains {len_x}"

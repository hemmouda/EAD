...
class Subject(Dataset):
    ...
    def __init__(
        self, data_vectors: List, spindle_vectors: List, phase: int, patient_id: str
    ) -> None:
        super(Subject, self).__init__()
        assert len(data_vectors) == len(spindle_vectors)

        assert data_vectors.shape == spindle_vectors.shape
    ...
    def preprocess(self):
        """
        Preprocess the data of the subject.
        """
        for i, data_block in enumerate(self.data):
            robust_scaler = RobustScaler()
            clamp_value = 20

            x = robust_scaler.fit_transform(data_block[:, None])
            x[x < -clamp_value] = -clamp_value
            x[x > clamp_value] = clamp_value

            self.data[i] = x.flatten()
...
class MODADataset(ConcatDataset[Subject]):
    ...
    preprocessing : bool
        If the data should be preprocessed using the RobustScaler before returned.
    augmentation : bool
        If data augmentation should be applied after preprocessing.
    """
    ...
    def __init__(
        ...
        # TODO: could be problematic when num_workers>0, but we are using num_workers=0 so it should be fine
        self.rng = np.random.default_rng()

    def augment_data(self, data, spindle):
        """
        Augment the given data by scaling it with a random factor. Only applied if `self.augmentation` is True.

        Parameters
        ----------
        data : ndarray
            The block of eeg data to be augmented; format (11500,).

        Returns
        -------
        augmented_data : ndarray
            The augmented data; format (11500,).
        """
        if self.augmentation:
            # amplitude scaling
            factor_range = self.augmentation_params["scaling_factor_range"]
            if factor_range != 1:
                downscale = self.rng.choice([True, False])
                factor = self.rng.uniform(
                    factor_range if downscale else 1,
                    1 if downscale else 1 / factor_range,
                )
                data = data * factor
    ...
    def __getitem__(self, idx):
        ...
        data = self.augment_data(data, spindle)
...
class MODADataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        with open(self.config.data_file, "rb") as data_file:
            subjects = pickle.load(
                data_file
            )  # load the configured file containing the subjects
    ...
    def subjects_to_data_loader(self, subjects, shuffle=False, train=False):
        """
        ...
        train : bool
            If the data loader is used for training.
        ...
        dataset = MODADataset(
            ...
            augmentation=self.augmentation and train,
            augmentation_params=self.augmentation_params,
        )
        # use num_workers=0 as this is for some reason faster than using multiple workers
        return DataLoader(
            dataset, self.batch_size, shuffle, num_workers=0, pin_memory=True
        )

    def train_dataloader(self):
        return self.subjects_to_data_loader(
            self.subjects_train, shuffle=True, train=True
        )
    ...
    def test_dataloaders(self):
        ...
        if type(self.subjects_test[0]) is list:  # multiple test datasets
            return [
                self.subjects_to_data_loader(test_phase)
...
class MODADataModuleCV(MODADataModule):
    ...
    def prepare_data(self):
        ...
        self.subjects_train = sum(
            [subjects[cv_folds[i]] for i in range(k) if i != self.fold_idx], []
        )
...
def create_subjects(eeg_files, spindle_files, block_files):
    ...
    subjects = []
    for idx, (eeg_file, spindle_file, block_file) in enumerate(
        zip(eeg_files, spindle_files, block_files)
    ):
        ...
        subjects.append(
            np.array(
                [
                    Subject(
                        subject_data_vector,
                        subject_spindle_vector,
                        phase=idx,
                        patient_id=subject_per_block[block_idx],
                    )
                    for subject_data_vector, subject_spindle_vector, block_idx in zip(
                        data_vectors_per_subject,
                        spindle_vectors_per_subject,
                        unique_indices,
                    )
                ],
                dtype=Subject,
...
def create_train_test(subjects, test_size, only_small_subjects_in_test=True):
    ...
    assert (
        0 <= test_size <= 1
    ), "the test_size has to be between 0 and 1 (both inclusive)"

    n_subjects = sum([len(s) for s in subjects])
    n_test = int(n_subjects * test_size)
    n_test_per_phase = n_test // len(
        subjects
    )  # absolute size of test data equal in all cohorts
    ...
    for phase, subjects_phase in enumerate(subjects):
        ...
        subjects_train, subjects_test = train_test_split(
            subjects_phase[mask], test_size=n_test_per_phase
        )
        if only_small_subjects_in_test:
...
def create_train_val(subjects, val_size):
    ...
    subjects_train, subjects_val = train_test_split(
        subjects, test_size=val_size, stratify=phases
    )
...
def create_train_val_test(subjects, test_size, val_size, only_small_subjects=True):
    ...
    subjects_train, subjects_test = create_train_test(
        subjects, test_size, only_small_subjects
    )
...
def create_cv_folds(subjects, k):
    ...
    for (train_fold_idx1, test_fold_idx1), (train_fold_idx2, test_fold_idx2) in zip(
        generator1, generator2
    ):
        # combine the two corresponding splits
        train_fold = np.concatenate(
            (subjects1[train_fold_idx1], subjects2[train_fold_idx2])
        )
        test_fold = np.concatenate(
            (subjects1[test_fold_idx1], subjects2[test_fold_idx2])
        )

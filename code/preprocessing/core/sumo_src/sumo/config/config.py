...
class Config:
    ...
    def __init__(self, experiment: str, create_dirs=True, config_group=None):
        ...
        if config_data:
            self.data_dir = base_dir / Path(config_data["directory"])
            ...
            assert (self.train_split_name is None) == (
                self.val_split_name is None
            ), "either train and validation splits are given or none of them when using cross validation"
            assert (
                sum(
                    [self.val_split_name is not None, self.cross_validation is not None]
                )
                == 1
            ), "either train and validation splits are given or none of them when using cross validation"
            ...
            if self.train_split_name is not None:
                assert (
                    self.train_split_name in subjects
                ), "data file does not contain given train split"
                assert (
                    self.val_split_name in subjects
                ), "data file does not contain given validation split"
            else:
                ...
                for fold in self.cross_validation:
                    assert (
                        fold in subjects
                    ), f"data file does not contain given cross validation split {fold}"

            self.test_split_name = config_data["split"]["test"]
            if self.test_split_name is not None:
                assert (
                    self.test_split_name in subjects
                ), "data file does not contain given test split"

            self.batch_size = config_data["batch_size"]
            assert (
                type(self.batch_size) is int and self.batch_size >= 1
            ), "the batch size has to be at least one"
            self.preprocessing = config_data["preprocessing"]
            assert type(self.preprocessing) in [bool, type(None)]
            self.augmentation = config_data["augmentation"]
            assert type(self.augmentation) in [bool, type(None)]
            self.augmentation_params = config_data["augmentation_params"]
            assert type(self.augmentation_params) in [dict, type(None)]
        ...
        assert self.float32_matmul_precision in [
            "medium",
            "high",
            "highest",
        ...
        assert (
            type(self.n_classes) is int and self.n_classes >= 1
        ...
        assert (
            type(self.depth) is int and self.depth >= 1
        ), "the depth of the model has to be at least one"
        self.channel_size = config_model["channel_size"]
        assert (
            type(self.channel_size) is int and self.channel_size >= 1
        ...
        assert (
            len(self.pools) == self.depth
        ), "the number of given pooling sizes does not match the specified depth"
        for p in self.pools:
        ...
        assert (
            type(self.moving_avg_size) is int and self.moving_avg_size >= 1
        ...
        assert (
            type(self.n_epochs) is int and self.n_epochs >= 1
        ), "the number of training epochs has to be at least one"
        ...
        if type(self.early_stopping) is int:
            ...
            ), "the number of epochs used for early stopping has to be at least one"
        ...
        ), "optimizer must be a subtype of torch.optim.Optimizer"
        ...
        if config_lr_scheduler is None:
            ...
            self.lr_scheduler = getattr(
                optim.lr_scheduler, config_lr_scheduler["class_name"]
            )
        ...
        try:
            ...
            self.loss = getattr(
                losses, config_loss["class_name"]
            )  # first try to import a custom loss function
        except AttributeError:
            self.loss = getattr(
                nn, config_loss["class_name"]
            )  # if none (matching) exists import a pytorch function
        ...
        assert (
            type(overlap_threshold_step) is float
            and 0.0 < overlap_threshold_step <= 1.0
        ), "the step size of the used overlap thresholds has to be greater than zero and not more than one"
        self.overlap_thresholds, step = np.linspace(
            0.0, 1.0, int(1 / overlap_threshold_step) + 1, retstep=True
        )
        assert (
            step == overlap_threshold_step
        ...
        if config_group is None:
            ...
            self.output_dir = base_dir / "output" / config_group / self.run_name
